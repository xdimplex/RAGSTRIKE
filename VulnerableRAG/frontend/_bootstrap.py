...
DEFAULT_PROFILE = "vulnerable"

#: Display name and icon per profile. The frontend is deliberately one codebase serving both labs
#: (ADR-009), so the name has to be DERIVED rather than typed into each page. Pages used to hardcode
#: "VulnerableRAG", which meant the hardened twin introduced itself as the vulnerable one.
_BRANDING = {
...
def page_title(settings: Settings, section: str = "") -> str:
    """Browser tab title: ``"Chat · VulnerableRAG"``, or just the app name on the home page."""
