...
Builds an app for a given profile. This is the same factory VulnerableRAG uses; the difference
between the two applications arrives entirely through the engine's policy chain, never through a
branch here.

SecureRAG adds two middlewares that VulnerableRAG does not install -- security headers and the
rate-limit counter -- and imports its own profile builder. Those are the entire diff in this file,
and ``docs/compatibility-guide.md`` lists them.
...
DESCRIPTION = """
A **hardened** Retrieval-Augmented Generation application, and the defended half of a security lab.

It exposes exactly the same REST API as VulnerableRAG, runs exactly the same pipeline, and answers
benign questions equivalently. The difference is the security control chain: input validation,
context sanitization, retrieval filtering, prompt hardening, output validation, secret masking, and
citation grounding. See `docs/security-features.md`.

**Hardened is not audited.** This is a lab application that ingests a synthetic corpus with planted
canaries. Run it on loopback, and never with real data.
"""
...
def create_app(
    *,
    ...
    """
    # Imported here rather than at module scope, matching VulnerableRAG's factory: this import is
    # the one profile-specific line, and keeping it local keeps the factory itself profile-agnostic.
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        log.info(
            "%s profile ready on the API -- %d security policies active: %s",
            engine.profile.upper(),
            len(engine.policies),
            ", ".join(policy.name for policy in engine.policies.policies),
        )
        yield

    app = FastAPI(
        title=f"SecureRAG ({profile})",
    ...
    # Starlette runs the last-added middleware outermost, so security headers are attached to
    # every response -- including one produced by an error handler.
    app.add_middleware(RateLimitPlaceholderMiddleware)
    if engine.settings.security.http.security_headers:
...
def _install_error_handlers(app: FastAPI) -> None:
    ...
    async def _policy_rejected(request: Request, exc: PolicyRejectionError) -> JSONResponse:
        # The live refusal path. VulnerableRAG wired this handler in Phase 1 and never reaches it;
        # here it is how InputValidator returns a 400 carrying a caller-safe reason.
        return JSONResponse(
            status_code=400,
