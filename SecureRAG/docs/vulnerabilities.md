# Vulnerability Catalogue — *countered here*

> ⚠️ **This file is inherited from VulnerableRAG and describes weaknesses that are NOT present in
> this repository.** It is kept because SecureRAG's entire reason to exist is being the same
> application without them, and the catalogue is the index the regression suite is organised around:
> `tests/regression/test_regression.py` has one test per weakness below, each asserting it is absent.
>
> Read it as a specification of what was defended against, not as a description of this application.
> [`security-features.md`](security-features.md) describes what actually runs here, and
> [`architecture-comparison.md`](architecture-comparison.md) shows the two side by side.
